...
const confirmDelete = (url) => {
    return new Promise((resolve, reject) => {
        swal({
            title: "Are you sure?",
            text: "Your record will be deleted permanently!",
            icon: "warning",
            buttons: true,
            dangerMode: true,
        }).then((willDelete) => {
        ...
        fetch(url, {
            ...
            .then((data) => {
                if (data.status === "200") {
                    ...
                    if (typeof window[data.functionName] === "function") {
                        if (window[data.functionName].length === 0) {
...
document.addEventListener("click", (e) => {
    ...
    if (target.classList.contains("button.password-addon"))
